function [spindle_det] = LSM_spindle_detections(spindle_probabilities, hdr)

  prob_threshold = 0.95;
  Fs = hdr.info.sfreq;
  
  min_spindle_duration    = 0.5; % [s]
  max_spindle_duration    = 2;
  blankout_duration       = 1; % [s]
  spindle_separation_threshold = 1;   % [s]
  
  spindle_det = [];
  for k=1:length(spindle_probabilities)         % For each electrode,
  %k=1;
  
      %%%% 1) Get initial set of detections. ------------------------------
      prob = spindle_probabilities(k).prob;                             % Get probability for this electrode.
      t    = spindle_probabilities(k).t;                                % Get time for this electrode.
